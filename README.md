...
### 1. 유저 입력
  - 메인 화면에서의 입력
    - 1 과 Q 둘중 하나이다
    - 1 과 Q 둘 중 하나가 아니면, 에러를 발생시킨다.
      - 이때, [ERROR] 를 붙여서 출력해야 한다.
  
  - 경로 기준에서의 입력
    - 1,2,B 중에서 입력받아야 한다
    - 1,2,B 를 제외한 다른 입력시 에러를 발생시킨다.
      - 이때, [ERROR] 를 붙여서 출력해야 한다.

  - 1,2 선택 시 출발, 도착역 입력
    - 출발역 과 도착역을 입력받는다. 
    - 전체 역 Repo 안에 없으면 에러를 발생시킨다.
      - 없는 역을 입력받으면 에러를 발생시킨 후, 다시 경로 기준으로 돌아간다.
    - 같은 역을 입력받으면 에러를 발생시킨 후, 다시 경로 기준으로 돌아간다.
     - 이때, [ERROR] 를 붙여서 출력해야 한다.

### 2. 유저 출력
  - 선택 화면
    - "##원하는 기능을 선택하세요." 를 출력해야 한다.
  - 시작 화면
    - 반드시 메인 화면을 출력해야 한다.
    - 1과 Q로 메인화면을 구성한다. 
      - 1을 입력받으면 원하는 기능을 선택하게 한다.
      - Q 를 입력받으면 프로그램을 종료한다 

  - 경로 기준
    - 반드시 해당 화면을 구성한다
    - 1,2,B 로 화면을 구성해야 한다
      - 1 와 2 를 입력받으면 출도착역을 입력받는 부분으로 넘어간다 
      - B 를 입력받으면 앞으로 돌아가야 한다.
    - 1,2,B 를 제외한 다른 입력을 받을 시 에러를 발생시킨다.
      - [ERROR] 를 붙여서 출력해야 한다.
  
  - 조회 결과
    - "## 조회 결과" 뒤에 모든 줄은 모두 [INFO] 를 붙여서 출력해야 한다.
    - 총 거리와 총 소요시간 앞에는 "---" 를 넣어서 출력한다
...
### 4. StationRepository
  - 역들이 모두 모여 있는 Repository 이다.
  - 여기에 없는 역을 입력받으면 에러 메세지를 출력해야 한다.

### 5. Line
  - 노선과, 해당 노선의 역들이 모여 있는 정보가 들어 있다.
  - 각 역 간의 시간,거리 정보도 같이 들어 있다.

### 6. LineRepository
  - 각 노선의 정보들이 모두 모여 있는 Repository 이다.


### 7. AllMapRepository
  - 모든 노선의 정보 graph 들을 담고 있는 Repository 이다.
  - 시간 기준, 거리 기준으로 나눠서 관리된다.
  - jgrapht 라이브러리를 활용한 graph 를 생성하고, 정보를 담는다.
  - 주로 Controller 안에서 직접 정보를 들고 오는 객체이다.
  
### 8. 초기화
  - 반드시 처음 실행되는 부분에서 정보가 모두 초기화되어야 한다.
    - 거리와 소요 시간은 양의 정수로 , 단위는 km, 분을 사용한다고 한다
  
  1. 역 정보
    - 교대역, 강남역, 역삼역, 남부터미널역, 양재역, 양재시민의숲역, 매봉역

  2. 노선 정보
    - 지하철 노선으로 2호선,3호선,신분당선이 존재한다.  

  3. 노선에 정보를 등록한다.
      + 해당하는 Line 객체가 없으면 객체를 생성하고, 정보를 입력해 준다.
      + 2호선: 교대역-( 2km / 3분 )-강남역-( 2km / 3분 )-역삼역
      + 3호선: 교대역-( 3km / 2분 )-남부터미널역-( 6km / 5분 )-양재역-( 1km / 1분 ) - 매봉역
      + 신분당선: 강남역 - ( 2km / 8분 ) - 양재역 - ( 10km / 3분 ) - 양재시민의숲역
      + 각 Line 에, 위의 정보가 입력되어야 한다.
